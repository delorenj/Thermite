...
int XYZCompare(const void *v1, const void *v2)
{
	...
	int CreateDelaunayTriangulation(b2Vec2 * verts, int n_verts, b2Vec2* extraPoints, int pointCount, b2Vec2* triangles)
	{
		...
		//b2Vec2 *tris = new b2Vec2[triangleCount*3];
		for (int i=0;i<triangleCount;i++)
		{
			triangles[3*i] = b2Vec2(p[v[i].p1].x,p[v[i].p1].y);
			triangles[3*i+1] = b2Vec2(p[v[i].p2].x,	p[v[i].p2].y);
			triangles[3*i+2] = b2Vec2(p[v[i].p3].x,	p[v[i].p3].y);
			//tris[i] = b2Triangle(p[v[i].p1].x,	p[v[i].p1].y, p[v[i].p2].x, p[v[i].p2].y, p[v[i].p3].x, p[v[i].p3].y);
		}
